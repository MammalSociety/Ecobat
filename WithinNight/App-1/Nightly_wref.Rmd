...
**The average number of bat passes of each species per night throughout each month. Calculated with (Number of passes each month) / (Number of survey nights each month). Values are given to 1 decimal place.**
...
## Median Nightly Bat Passes for each Month 
# Per Detector
...
`r if("Bat_time" %in% colnames(All_data) == TRUE) {'##### Page Break## Time (Seconds) Bat Present\n **Calculated from...**\n\n'}`
...
for (i in (unique(All_data$Spp))) {
  print(Table3 %>%
        ...
        theme(axis.text.x = element_text(size=10, hjust=1, vjust=0.5,
                                   colour = "black")) +
...
print(ggplot(All_data, aes(x=post_set_min, y=Spp, colour=Spp)) +
  ...
  theme_bw() +
  theme(legend.position = "none") +
  theme(plot.caption = element_text(colour = "black", size = 17)) +
  theme(strip.text.x = element_text(size=34, face="bold")) +
  theme(axis.title.y = element_blank()) +
  theme(axis.title.x = element_text(colour = "black", size = 17,
                                     face = "bold")) +
  theme(axis.text.x = element_text(size = 14, hjust=0.5, vjust=1,
                                   colour = "black",
                                   face = "bold")) +
...
`r if ((is.element("15-Jun", sun_data$Day) == TRUE) || (is.element("30-Jul", sun_data$Day) == TRUE)) {'##### Page Break###Bat Passes Potentially Indicating Close Proximity to a Roost (Maternity Period Only)\n*Maternity period defined as 15th June - 30th July*\n\n'}`
