...
```{r, echo=FALSE, fig.cap="This report was produced free of charge by the Mammal Society to support evidence-based conservation of bats.", out.width = "8\\linewidth"}
...
## Bat Passes per Month 
# Survey Effort
**The number of survey nights per month.**
...
**The average number of bat passes of each species per night throughout each month. Calculated with (Number of passes each month) / (Number of survey nights each month). Values given to 3 significant figures.**
...
Table3 <- left_join(Table, Table2, by = "Month") %>%
  ...
  select(Month, Spp, Detector.ID, Effort) %>%
  spread(Month, Effort) %>%
...
for (i in unique(All_data$Spp)) {
  ...
  geom_text(aes(y=(max(n)+1), x=Month, label=Detector.ID, group=Detector.ID), position=position_dodge(0.8), size=4) +
  theme_bw() +
...
```{r fig.width=22, fig.height=15, fig.align="center"}
