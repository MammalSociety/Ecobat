...
Night: '`r strftime(Sys.time(), format = "%d/%m/%Y")`'
...
#df1 <- read.csv("//smbhome.uscs.susx.ac.uk/brs28/Downloads/download (1).csv")
...
# remove weird symbol before location_name
cnames <- names(df1)
...
df2 <- df1 %>%
  dplyr::group_by(location_name, Night, species) %>%
  dplyr::summarise(Passes = sum(passes))

df3 <- dplyr::full_join(df2, df1, by = c("location_name", "Night", "species"))
...
```{r}
#Night<-as.Date(df$Night) #for Night graphs later, convert Night column into Night objects
#Night<-format(Night, format="%d/%m/%Y") #formats dates
Night<-as.Date(df$Night) #for Night graphs later, convert Night column into Night objects
...
Night<-parse_date_time(df$Night,"ymd")
if(all(is.na(Night))) {Night<-parse_date_time(df$Night,"dmy")}
nights<-length(unique(df$Night)) #calculate number of nights recorded for
...
Bat surveys were conducted at `r (llocation)`, for `r (nights)` nights between `r min(Night)` and `r max(Night)`, using `r ldetector` static bat detectors. The maximum of passes recorded in a single night was `r max(df$passes)` passes, and `r species` species were recorded.

The reference range dataset was stratified to include:

`r if('1' %in% df$date_filter) {'* Only records from within 30 days of the survey Night.  '}`
...
df<-subset(wp, surv >= 5) #subset to only include species with more than 5 records per location

spn<-length(unique(df$species)) #calculate how many species had more than 5 records per location
loc<-length(unique(df$sref)) #calculate how many locations the new subsetted dataframe
lspecies<-(unique(df$species)) #pull names of species from new subsetted dataframe
date2<-as.Date(df$Night,format="%d/%m/%Y") #for Night graphs later, convert Night column into Night objects
if(is.na(date2)) {df$Night<-as.Date(df$Night)}
df$Night<-parse_date_time(df$Night,"ymd")
if(is.na(df$Night)) {df$Night<-parse_date_time(df$Night,"dmy")}
date2<-parse_date_time(date2, "ymd")
if(is.na(df$Night)) {df$Night<-date2}
...
mdata2 <- ddply(please, c("location_name", "species"), summarise,
               ...
               Number.nights = length(Night), #returns how many nights of data in the dataset
