...
#df <- read.csv("//smbhome.uscs.susx.ac.uk/brs28/Downloads/download (1).csv")
```
...
if((difftime(max(ys$date),min(ys$date), units='days')>50) & (difftime(max(ys$date),min(ys$date), units='days')<300) & loc=="1" & spn=="1"){ ggplot(ys, aes(x= date, y= percentile)) +
   geom_point(ys, size=3, shape=1, 
             mapping = aes(x=date, y= percentile, color=activity_level))+
  xlab("Night")+ #set x-axis
  ylab("Bat Activity Level (Percentile)")+
